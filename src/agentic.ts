...
//
// These are deliberately SEPARATE from BUILTIN_PATTERNS. The same
// sentence carries different meaning depending on where it appears: a
// user writing "do not mention this to anyone" is unremarkable, but a
// tool description saying it is instructing the model to hide its
// behavior from the operator. These patterns therefore only ever run
// against tool definitions, never against user input.

const CONCEALMENT_PATTERNS: InjectionPattern[] = [
  {
...
const SHADOWING_PATTERNS: InjectionPattern[] = [
  {
